...
type AgesomaInitialState = {
  greeting: string;
  brief: string;
  activeWork: number;
  verifiedResults: number;
  verifiedValue: string;
  approval: AgesomaApproval | null;
};
...
export default function AgesomaClient({ initial }: { initial: AgesomaInitialState }) {
  ...
  return (
    <main className="agesomaShell">
      ...
      <section className="agesomaStage" aria-live="polite">
        <div className={pending ? "agesomaOrb isThinking" : listening ? "agesomaOrb isListening" : "agesomaOrb"} aria-label={pending ? "AGESOMA pensando" : listening ? "AGESOMA ouvindo" : "AGESOMA"}>
          <span className="agesomaOrbLobe agesomaOrbLobeA" />
      ...
      <section className="agesomaConversation">
        {messages.map((message) => (
          <article key={message.id} className={`agesomaMessage ${message.role}`}>
            <span className="agesomaMessageRole">{message.role === "assistant" ? "AGESOMA" : "VOCÊ"}</span>
            <div className="agesomaMessageText">{message.text}</div>
            {message.approval ? (
              <div className="agesomaApproval">
        ...
        {pending ? (
          <article className="agesomaMessage assistant isPending">
            <span className="agesomaMessageRole">AGESOMA</span>
            <div className="agesomaThinkingDots"><i /><i /><i /></div>
          </article>
        ) : null}
      </section>
