...
export default async function Home() {
  ...
  const primary = firstApproval ? {
    ...
    title: approvals.length === 1 ? "Tenho uma decisão pronta para você." : `Tenho ${approvals.length} decisões prontas para você.`,
    body: objective(firstApproval.payload),
    action: "Revisar agora",
    href: "#decisoes"
  } : running ? {
    eyebrow: "Em andamento",
    title: "Estou cuidando disso agora.",
    body: objective(running.payload),
    action: "Ver andamento",
    href: "#atividade"
  } : firstOpportunity ? {
    eyebrow: "Encontrei algo",
    ...
    body: "Só entra aqui o que puder ser confirmado.",
    action: "Ver resultados",
    href: "#resultados"
  } : {
    eyebrow: "AGESOMA",
    title: "O que você quer resolver hoje?",
    body: "Peça como pediria a alguém da sua equipe. A AGESOMA organiza o trabalho e chama você somente quando uma decisão realmente importa.",
    action: "Pedir algo",
    href: "/onboarding"
  };

  const next = firstApproval
    ? { title: "Revisar a decisão pendente", body: "Veja o contexto e decida sem precisar administrar a tecnologia por trás.", href: "#decisoes", label: "Revisar" }
    : firstOpportunity
      ? { title: "Decidir se vale agir agora", body: "Veja o que foi encontrado e transforme em trabalho somente se fizer sentido.", href: "#oportunidades", label: "Abrir" }
      : running
        ? { title: "Deixar o trabalho seguir", body: "Você será chamado se surgir uma mudança relevante ou uma decisão sua.", href: "#atividade", label: "Acompanhar" }
  ...
  return (
    <main className="productShell" id="inicio">
      <header className="topbar">
        ...
        <a className="primaryCta" href="/onboarding">Pedir algo</a>
      </header>

      <nav className="nav" aria-label="Navegação principal">
        <a className="navItem active" href="#inicio">Início</a>
        <a className="navItem" href="#atividade">Trabalho</a>
        <a className="navItem" href="#resultados">Resultados</a>
        <a className="navItem" href="#conexoes">Conexões</a>
      </nav>
      ...
      <section className="r3ActionHero">
        <div className="r3ActionHeroCopy">
          ...
          <div className="heroActions">
            ...
            <span className="heroTruth">Você acompanha o que importa sem administrar agentes, prompts ou automações.</span>
          </div>
        </div>
        ...
        {running ? (
          <aside className="r3LiveIntelligence">
            ...
            <div>
              ...
              <p>Há trabalho real em andamento.</p>
            </div>
          </aside>
        ) : (
          <aside className="r3QuietMaterial">
            <div>
              ...
              <strong>{workspace ? "Tudo sob controle." : "Pronto quando você estiver."}</strong>
              <p>{workspace ? "Nenhuma execução ativa neste momento." : "Faça um pedido para começar."}</p>
            </div>
          </aside>
        )}
      </section>
      ...
      <section className="r3MetricStrip" aria-label="Resumo">
        ...
        <div><span>Encontrei</span><strong>{opportunities.length}</strong></div>
      </section>

      <section className="workspaceSection" id="atividade">
        <div className="sectionIntro">
          <div className="eyebrow">Atividade</div>
          <h2>Você vê o trabalho, não a complexidade.</h2>
          <p>O que está acontecendo aparece em linguagem de negócio, com contexto suficiente para agir.</p>
        </div>
        <div className="workspaceStack">
          <article className="workspaceRow">
            <span className="workspaceLabel">Agora</span>
            <div><strong>{activeTasks.length ? `${activeTasks.length} trabalho${activeTasks.length === 1 ? "" : "s"} em andamento` : "Nada em andamento"}</strong><p>{activeTasks[0] ? objective(activeTasks[0].payload) : "Seu próximo pedido aparece aqui quando começar."}</p></div>
            <span className="quietStatus">{activeTasks.length || "—"}</span>
          </article>
          <article className="workspaceRow" id="decisoes">
            <span className="workspaceLabel">Decisões</span>
            <div><strong>{approvals.length ? `${approvals.length} esperando por você` : "Nenhuma decisão pendente"}</strong><p>{firstApproval ? objective(firstApproval.payload) : "Você só será chamado quando uma escolha realmente precisar ser sua."}</p></div>
            <span className="quietStatus">{approvals.length || "—"}</span>
          </article>
          <article className="workspaceRow" id="oportunidades">
            <span className="workspaceLabel">Encontrei</span>
            <div><strong>{firstOpportunity?.title ?? "Nada novo por enquanto"}</strong><p>{firstOpportunity?.summary ?? "Quando aparecer algo relevante para o negócio, você verá aqui."}</p></div>
      ...
      <section className="resultSection" id="resultados">
        <div className="sectionIntro">
          ...
          <h2>{verifiedCount ? `${money(outcomes?.net_value_cents)} de impacto líquido confirmado.` : "Resultados só aparecem quando podem ser comprovados."}</h2>
          <p>{verifiedCount ? `Receita atribuída: ${money(outcomes?.attributed_revenue_cents)} · custo registrado: ${money(outcomes?.total_cost_cents)}.` : "Sem estimativas apresentadas como fato."}</p>
        </div>
        <div className="workspaceCard">
          ...
          <p>{lastArtifact ? "A entrega permanece ligada ao trabalho que a produziu." : "Quando o primeiro trabalho terminar, o resultado fica disponível aqui."}</p>
        </div>
      </section>

      <section className="resultSection" id="conexoes">
        <a className="workspaceCard" href="/onboarding">
          <span className="workspaceLabel">Conexões</span>
          <strong>Escolher onde o trabalho acontece →</strong>
          <p>Conecte uma fonte somente quando ela for necessária para o próximo trabalho.</p>
        </a>
        <a className="workspaceCard" href="#atividade">
          <span className="workspaceLabel">Histórico</span>
          <strong>Ver o que está acontecendo →</strong>
          <p>Acompanhe trabalho, decisões e entregas sem abrir a infraestrutura por trás.</p>
        </a>
      </section>
    </main>
  );
