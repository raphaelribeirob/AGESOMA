...
- [ ] Every Hermes network egress passes through an AGESOMA-controlled system-level policy boundary. Application-level egress decisions are recorded, but a physical network proxy/VM boundary is still required for the target isolation level.
...
- [x] Consumer-grade guided onboarding exists.
